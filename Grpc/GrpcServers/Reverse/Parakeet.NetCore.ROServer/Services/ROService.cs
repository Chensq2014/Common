...
namespace Parakeet.NetCore.ROServer.Services
{
    /// <summary>
    /// 设备反控接收服务Service
    /// </summary>
    public class ROService : ReverseCommand.ReverseCommandBase
    {
        ...
        public override async Task<ReverseReply> Execute(ReverseRequest request, ServerCallContext context)
        {
            ...
            try
            {
                ...
                //将采集设备作为一种独立的设备类型
                if (request.Command.Equals(AppConstants.REGISTER_PERSON_COMMAND))
                {
                    device.Supplier.Code = AppConstants.REGISTER;
                }
